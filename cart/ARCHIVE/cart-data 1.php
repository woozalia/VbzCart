...
/* old code
	...
	// get the existing fields (for reading or comparison)

	$arVals = $this->CartValues();

	// now, are we READING or WRITING a field?
	...
	if (is_null($val)) {
		...
		return $arVals[$idType];
                ...
                $idCart = $this->CartID_master();
                $sType = self::NameFromIndex($idType);
                $sMsg = "Internal error: trying to access nonexistent field ($idType: $sType) in cart id=$idCart";
		throw new exception($sMsg);
		return NULL;
	    }
	} else {
