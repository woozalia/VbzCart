...
*/
class VCM_StockBins extends vcVbzTable_admin {
...
class VCM_StockBin extends vcVbzRecs_admin {
    protected $idEvent;
...
*/
    ...
    */
	...
	for all Places, with the current Place as the default
      HISTORY:
	2015-11-01 Apparently PlaceRecord() now returns a NEW record (instead
	  of NULL) when ID_Place is null, so now we test the record to see if
	  it is NEW rather than checking to see if the returned value is NULL.
    */
    protected function Place_DropDown($sName=NULL,$idPlace=NULL) {
	$rcPlace = $this->PlaceRecord();
	if ($rcPlace->IsNew()) {
	    $rsPlaces = $this->PlaceTable()->GetData_active();
	    return $rsPlaces->DropDown($sName,$idPlace);
	} else {
...
__END__;
    ...
    */
	...
	/*
	    if ($this->IsNew()) {
		$this->PageForm()->NewVals(array('WhenCreated'=>'NOW()'));
	    } else {
	...
	*/
	    $frm = $this->PageForm();
	    $out .= $frm->Save();	// save edit
	    $this->SelfRedirect();	// 2011-11-05 does "return" make this work now?
	    return;
	}
...
//	  new clsActionLink_option(array(),'inv',NULL,'inventory',NULL,'list all inventory of location '.$strName)
	...
	if ($doEdit) {
	    ...
	    $htPlace = $this->Place_DropDown('ID_Place',$idPlace);
	    //$htDescr = 	$oForm->RenderControl('Descr');
	    //$htNotes = 	$oForm->RenderControl('Notes');
	    $oForm->ControlObject('isEnabled')->FieldObject()->SetValueNative($doEnabled);	// set the enabled flag to save properly
	    $htForSale = $oForm->ControlObject('isForSale')->Render(TRUE);
	    $htForShip = $oForm->ControlObject('isForShip')->Render(TRUE);
	    $htEnabled = $oForm->ControlObject('isEnabled')->Render(TRUE); 
    ...
    */
	...
	if (is_null($this->oForm)) {
		...
		$oCtrl = new fcFormControl_HTML($oForm,$oField,array('size'=>30));
	      $oField = new fcFormField($oForm,'WhenCreated');
...
*/
    ...
    */
    public function AdminList($idPlace) {
