...
*/
class VCM_RstksRcvd extends clsTable {
    public function __construct($iDB) {
	parent::__construct($iDB);
	  $this->ClassSng('VC_RstkRcvd');
	  $this->Name(KS_TABLE_RESTOCK_RECEIVED);
	  $this->KeyName('ID');
	  $this->ActionKey('rstk-rcd');
    }
}
class VC_RstkRcvd extends clsDataSet {
...
__END__;
		...
		$yrWhen = $dtWhen->Year();
		$ftYear = $yrWhen;
		$ftWhenShip = Date_DefaultYear($rs->Value('WhenShipped'),$yrWhen);
		$ftWhenRecd = Date_DefaultYear($rs->Value('WhenReceived'),$yrWhen);
		$ftWhenPaid = Date_DefaultYear($rs->Value('WhenDebited'),$yrWhen);
