...
class VC_UserAcct extends clsVbzUserRec {
    ...
    */
	...
	$rcUser = $oApp->User();
	if ($rcUser->CanDo(KS_PERM_CUST_ADMIN) || $this->IsLoggedIn()) {
	    $out = $this->AdminPage();
	} else {
...
__END__;
	...
	if (empty($this->frmPage)) {
	    ...
	    $frmPage->AddField(new clsField('UserName'),		new clsCtrlHTML(array('size'=>20)));
	    $frmPage->AddField(new clsField('FullName'),		new clsCtrlHTML(array('size'=>40)));
