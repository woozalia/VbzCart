...
class vcqtItemsInfo extends clsItems {
    use ftQueryableTable;

    // ++ SETUP ++ //
/* 2016-10-26 replaced by SingularName()
    public function __construct($iDB) {
	parent::__construct($iDB);
	  $this->ClassSng('vcqrItemInfo');
    }
*/
    // -- SETUP -- //
    // ++ CEMENTING ++ //
...
//*/
    ...
    */
	...
	$qo->Select()->Source()->AddElements(
	  array(
	    new fcSQL_JoinElement($qsi,'si.ID_Item=i.ID')
	    )
	  );
...
class vcqrItemInfo extends clsItem {
    ...
    */
    ...
    throw new exception('Does anything call this? Debugging printouts need to be removed.');
	$ar = NULL;
	if ($this->HasRows()) {
    ...
    */
	...
	if ($nQty > 0) {
	    ...
	    $out .= "<tr class=$cssClass><td colspan=5>$sNoun $sDescr</td></tr>"
	      .clsItems::Render_TableHdr_forTitle()
	      ;
    ...
    */
	...
	if (is_null($dlrPrUse)) {
	    ...
	    $htCtrl = "<input size=3 name='$htCtrlName'>";
...
__END__;
	...
	2016-01-23 Moved from cls
    */
    private function AvailStatus() {
      $qtyInStock = $this->GetFieldValue('QtyForSale');
	if ($qtyInStock) {
	    $strCls = 'inStock';
	...
	$isInPrint = $this->GetFieldValue('isInPrint');
	if ($isInPrint) {
	    if ($this->GetFieldValue('isCurrent')) {
		    if ($qtyInStock) {
			$txt = $strStk.'; more available';
		    } else {
			$txt = KHTML_BACKORDER_ADJECTIVE_CLAUSE;
		    }
	    } else {
		if ($qtyInStock) {
