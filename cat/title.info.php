...
*/
class vctCatDepartments_queryable extends clsDepts {
...
trait vtQueryableTable_Titles {
	...
	return "LOWER(CONCAT_WS('/',s.CatKey,d.PageKey,t.CatKey))";
    }
...
class vcqtTitlesInfo extends vctShopTitles {
    ...
    */
	...
	$oj = $oq->Select()->Source();
	$oj->AddElement(new fcSQL_JoinElement($osItem,'si.ID_Item=i.ID'));
	...
	$qo->Select()->Fields(new fcSQL_Fields(array(
	      't.ID',
	      'QtyForSale' => 'SUM(si.Qty)'	// alias => source
	      )
	    )
	  );
...
FROM
	...
	LEFT JOIN
    `stk_items` AS si ON si.ID_Item = i.ID
        JOIN
    `stk_bins` AS sb ON si.ID_Bin = sb.ID

WHERE
    (tt.ID_Topic = $idTopic) AND (si.WhenRemoved IS NULL) AND (sb.isEnabled)
GROUP BY i.ID_Title
__END__;
    }
    /*----
    ...
    */
    public function SQO_forTopicPage_all($idTopic) {
	if (is_null($idTopic)) {
	    // include all Titles, then filter for no Topic
	    $sqlTopicJoin = 'LEFT JOIN';
	    $arTopicFilt = array('tt.ID_Topic IS NULL');
	...
	$sroTitle = $this->SQO_Source('t');
	$sroDept = new fcSQL_TableSource($this->DepartmentTable()->Name(),'d');
	$sroSupp = new fcSQL_TableSource($this->SupplierTable()->Name(),'s');
	$arJT = array(
...
FROM
    ...
    FROM
        stk_items AS si
    LEFT JOIN `stk_bins` AS sb ON si.ID_Bin = sb.ID) AS si ON si.ID_Item = i.ID
...
class vcqrTitleInfo extends vcrShopTitle {
    ...
    */
	...
	$rcImg = $this->ImageInfoQuery()->SpawnItem();
	$htImg = NULL;
	$sTitle = $this->RenderSummary_text();
	foreach ($arImRow as $idImg => $arImg) {
	    $rcImg->Values($arImg);
	    $htImg .= $rcImg->RenderInline_row($sTitle,clsImages::SIZE_THUMB);
	}
	...
	if (array_key_exists('@img',$row)) {	// if there's image data...
		...
		$htImg .= $rcImg->RenderSingle($sText,clsImages::SIZE_THUMB);
	    }
