...
//	  'DescText'	=> 'Descr',
    ...
    */
	...
	$qty = $this->QtyOrd();
	if ($qty != 0) {
	    $prcShPkg = $this->GetShippingPerPackage();
	} else {
	    // none of this item in package, so don't require this minimum
	    $prcShPkg = 0;
	}
	$arOut['sh-pkg'] = $prcShPkg;
	$arOut['sh-itm.qty'] = $this->GetShippingPerUnit() * $qty;
    ...
    */
	...
	In the future, users may be able to edit orders after submitting them... but there will be a design review first.
      HISTORY:
	2011-04-01 adapted from clsShopCartLine::RenderHtml() (now vcrCart::RenderStatic()) to clsOrderLine::RenderStatic()
    */
