...
*/
class vctCustCards extends vcBasicTable {
...
class vctCustCards_dyn extends vctCustCards {
    ...
    */
    ...
    public function CreateRecord($idBuyer, vcrCustAddr $rcAddr, vcCartData_Buyer $oBuyer) {
...
class vcrCustCard extends vcrContactCard_trait {
    ...
    */ /* 2016-07-31 Apparently no longer used.
	...
	return $this->GetSafeString().' '.$this->OwnerName();
    }
    public function CardTypeChar() {
	return vctCustCards::CardTypeChar($this->CardNumber());
    }
    public function CardTypeName() {
	return vctCustCards::CardTypeName($this->CardNumber());
    }
...
//	$this->OwnerName($rcAddr->NameString());	// $rcAddr does not have NameString yet
    ...
    public function Load_fromAddr(vcrCustAddr $rcAddr) {
    ...
    */
	...
	if (empty($sNum)) {
		...
		$arUpd = array(
		  ...
		  'CardSafe' => SQLValue($this->GetSafeString())
		  );
	    }
	}
	if ($bDoSave) {
