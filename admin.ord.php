...
/*
	$arLink = array(
	  'page'	=> $this->ActionKey(),
	  'show'	=> 'open');
	$wpOpen = $vgPage->SelfURL($arLink);
	$arLink['show'] = 'shut';
	$wpShut = $vgPage->SelfURL($arLink);
	$arLink['show'] = 'pulled';
	$wpPull = $vgPage->SelfURL($arLink);

	$arMenu = array(
	  'open'	=> array(
	    'path'	=> $wpOpen,
	    'popup'	=> 'show only open orders',
	    'displ'	=> 'open'),
	  'shut'	=> array(
	    'path'	=> $wpShut,
	    'popup'	=> 'show only closed orders',
	    'displ'	=> 'shut'),
	  'pulled'	=> array(
	    'path'	=> $wpPull,
	    'popup'	=> 'show only pulled orders',
	    'displ'	=> 'pulled'));
	$out = WikiSectionHdr('Orders','type',$arMenu);
*/
...
// dependent data //
    ...
    */
	...
	if ($objRow->hasRows()) {
		...
		//$wtNum = SelfLink_Page(KS_URL_PAGE_ORDER,'id',$id,$strNum);
		$wtNum = $strNum;
		...
		if ($iCalcStats) {
		    ...
		    if (is_array($arSum)) {
/*
			foreach ($arSum as $key => $val) {
			    $out .= " || [$key]=[$val]";
			} */
//echo '<pre>'.print_r($arSum,TRUE).'</pre>';
//die();
...
/* 2011-09-21 replacing with more friendly UI
    private function DoSetup() {
    // ACTION: Do the order setup
	global $vgPage,$vgOut;

	// get cart marked as source for this order
	// there should be only one, but the data schema allows more -- so check to be sure
	if ($strDo == 'cart') {
	    $out = $this->DoSetupCart(); // set up order from the selected cart
	} else {
	    $out = '';

	    $idCart_main = $this->ID_Cart;
	    if (is_null($idCart_main)) {
		$out .= '<br>This order does not have a default cart!';
		// FUTURE: Log an error with sysop alert
	    } else {
		$obj = $this->Cart();
		$out .= '<br>Default cart for this order:'.$obj->SetupLinks();
	    }
		...
		while ($objCarts->NextRow()) {
		    $id = $objCarts->KeyValue();
		    if ($id != $idCart_main) {
			$arOthers[$id] = $objCarts->Row;
			$cntOthers++;
		    }
		}
		if ($cntOthers > 0) {
		    $out .= "<br>Additional cart".Pluralize($cntOthers)." referring to this order:<ul>";
		    foreach ($arOthers as $id => $obj) {
			$out .= "\n<li> ".$obj->SetupLinks();
		    }
		    $out .= "\n</ul>";
		}
	    } else {
		$out .= '<br>No other carts refer to this order.';
		// This shouldn't happen, but logging isn't necessary since this is being viewed by an administrator.
		//$this->LogEvent('C0','No carts found for this order!');
	    }
...
/*
	...
	$objOrd = $this;
*/
//	$objCartTree = $objCart->GetDetailObjs();

	// get objects for the two contacts to be looked up and resolved:
...
// get fields which might need to be copied, depending on "ship_is" flags:
	...
	$idEmailRecip = $iCart->DataItem(KSI_CUST_SHIP_EMAIL);
	$idPhoneRecip = $iCart->DataItem(KSI_CUST_SHIP_PHONE);
	...
	if ($isShiptoSelf) {
	    ...
	    $idEmailBuyer = $iCart->DataItem(KSI_CUST_PAY_EMAIL);
	    $idPhoneBuyer = $iCart->DataItem(KSI_CUST_PAY_PHONE);
...
/* 2011-03-23 obsolete event management functions (...at least, I hope nothing is using them)
    public function StartEvent($iWhere,$iCode,$iDescr,$iNotes=NULL) {
	$arEvent = array(
	  'type'	=> clsEvents::kTypeOrdItem,
	  'id'		=> $this->ID,
	  'where'	=> $iWhere,
	  'code'	=> $iCode,
	  'descr'	=> $iDescr
...
class VbzAdminOrderTrxType extends clsDataSet {
    ...
    */
	...
	return $vgOut->InternalLink(KWT_DOC_TRX_TYPES.'/'.$this->Code,$txtCode,$this->Descr);
    }
    /*----
      HISTORY:
...
class VbzAdminOrderEvent extends clsEvent {
...
class clsOrderPullTypes extends clsTableCache {
	...
	while ($objRows->NextRow()) {
	    if ($objRows->ID == $iWhich) {
		$htSelect = " selected";
	    } else {
		$htSelect = '';
	    }
	    $out .= '<option'.$htSelect.' value="'.$objRows->ID.'">'.$objRows->Name.'</option>';
	}
	$out .= '</select>';
	return $out;
    }
