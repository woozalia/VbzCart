...
*/
/*::::
  PURPOSE: extends vctImages to handle store UI interactions
*/
class vctImages_StoreUI extends vctImages {
    
    // ++ OVERRIDES ++ //

    protected function SingularName() {
	return 'vcrImage_StoreUI';
    }
    
    // -- OVERRIDES -- //
        
}
class vcrImage_StoreUI extends vcrImage {
...
__END__;
    ...
    */
	...
	(Was originally in vcrImage, where skin was inaccessible.)
      HISTORY:
	2012-07-14 moved from vcrImage to vcrImage_StoreUI
...
// render the output
	if ($doNav) {
	    $htTitle = '<tr><td colspan=3 class=image-links>'
	      .$rcTitle->ShopLink('ordering page')
	      .'</td></tr>'
	      ;
	    $out = <<<__END__
...
__END__;
	...
	$doAbsolute = TRUE: prepend the Title's URL; FALSE: just return a link relative to the Title's URL.
      TODO: Rename RenderPageLink() to ShopLink()
	...and consider whether this is actually the right way to do it.
    */
...
//	}
	...
	if ($doAbsolute) {
	    $sFldr = $this->TitleRecord()->ShopURL().'/'.$sFldrRel;
	} else {
	    $sFldr = $sFldrRel;
	}
